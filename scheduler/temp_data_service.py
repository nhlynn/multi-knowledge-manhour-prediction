...
def _row_to_stash(row) -> dict[str, Any]:
    ...
    return {
        ...
        "projectRemark": (
            (row["project_remark"] if "project_remark" in keys else "")
            or data.get("projectRemark", "")
        ),
    }
    ...
    def add_stash(
        ...
        *,
        ...
        record = {
            ...
            # Project-level rich-text remark (Infrastructure Team only).
            # Stored both in its own column and inside json_data so a
            # restore has it regardless of which the reader prefers.
