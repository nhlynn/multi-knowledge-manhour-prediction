...
``services/excel_parser.py``. Built directly on top of the git-tracked
sanitized sample workbook, ``import/sgl/sgl_import_template.xlsx`` --
export's base template, the same public file Template Download serves
and import validation accepts (generated by
``import/sgl/build_sample_template.py`` from SGL Team's real internal
workbook, and structurally identical to it: sanitization only blanks
sample cell data, never the sheet/column layout the exporter writes
into). This module always copies that sample
...
the copy. The real customer workbook under ``simple_resource/``
(git-ignored) is no longer read anywhere at runtime, so export works on
a clean checkout.
...
def _strip_legacy_workbook_bloat(wb: openpyxl.Workbook) -> None:
    ...
    SGL Team's workbook lineage (the real internal template, and the
    git-tracked sanitized sample ``import/sgl/sgl_import_template.xlsx``
    generated from it -- the export base this module now loads) is an
    old corporate workbook that has accumulated thousands of "zombie"
    ...
    so without this step every single SGL export silently balloons in
    size and can trigger Excel's "we found a problem with some content"
...
def build_sgl_workbook(
    ...
    Args:
        ...
        template_path: Path to SGL Team's export base template
            (``import/sgl/sgl_import_template.xlsx`` -- the git-tracked
            sanitized sample; see ``SglExportBuilder.template_path``).
...
class SglExportBuilder(BaseExportService):
    ...
    def template_path(app_root_path: str) -> str:
        """Path to SGL Team's export base template.

        Returns the git-tracked sanitized sample template
        ``import/sgl/sgl_import_template.xlsx`` -- the same public file
        Template Download serves and import validation accepts. It
        shares the exact sheet/column STRUCTURE of SGL Team's real
        ...
        """
        return os.path.join(app_root_path, "import", "sgl", "sgl_import_template.xlsx")
    ...
    def fixed_phase_labels(app_root_path: str) -> list[str]:
        ...
        Reads the same git-tracked sample template
        ``template_path`` now points at
        (``import/sgl/sgl_import_template.xlsx``) -- it shares the exact
        sheet/column STRUCTURE of SGL Team's real internal workbook
        (sanitization only blanks data), so the phase labels are
        identical. This keeps Preview's fixed phases working on a clean
        checkout where ``simple_resource/`` (the customer workbook) isn't
        present. Returns [] only if the template can't be read."""
        path = SglExportBuilder.template_path(app_root_path)
        if os.path.isfile(path):
            try:
                wb = openpyxl.load_workbook(path)
                if SGL_SHEET_NAME in wb.sheetnames:
                    ws = wb[SGL_SHEET_NAME]
                    headers = _resolve_header_columns(ws)
